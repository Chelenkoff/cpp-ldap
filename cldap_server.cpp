...
bool Ldap::Server::Connect(const std::string & uri, bool ssl)
{
    ...
    if(ldap_object)
    {
        ldap_set_option(ldap_object, LDAP_OPT_PROTOCOL_VERSION, &protocol_version);

        if( ssl )
        {
            // Disable ssl certificate verification

            // LDAP_OPT_X_TLS_NEVER - This is the default. slapd will not ask the client for a certificate. (edit /etc/openldap/ldap.conf, add line: TLS_REQCERT never )
            // LDAP_OPT_X_TLS_ALLOW - The  client certificate is requested. If no certificate is provided, the session proceeds normally. If a bad certificate is provided, it will be ignored and the session proceeds normally.
            // LDAP_OPT_X_TLS_TRY - The client certificate is requested. If no certificate  is  provided, the session proceeds normally. If a bad certificate is provided, the session is immediately terminated.
            // LDAP_OPT_X_TLS_HARD and LDAP_OPT_X_TLS_DEMAND - These keywords are all equivalent, for compatibility reasons. The client certificate is requested.If no certificate is  provided, or a bad certificate is provided, the session is immediately terminated.

            int nTLSVerifyClient  = LDAP_OPT_X_TLS_NEVER;
            ldap_set_option (ldap_object, LDAP_OPT_X_TLS_REQUIRE_CERT, &nTLSVerifyClient );
        }
    }
